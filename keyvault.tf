...
locals {
  kv_secret_name = lower(replace("${local.system_name_short}-mysql-${local.server_name}-master", "/[^a-zA-Z0-9-]/", "-"))
  master_credentials = {
...
resource "azurerm_key_vault_secret" "master_credentials" {
  ...
  tags = merge(local.all_tags, {
    "mysql-server" = local.server_name
    "mysql-db"     = local.database_name
  })
}
