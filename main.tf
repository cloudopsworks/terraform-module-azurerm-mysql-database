...
locals {
  ...
  database_name = try(var.settings.database_name, "mysql")
  admin_login   = try(var.settings.administrator_login, "adminuser")
}

resource "azurerm_resource_group" "this" {
  name     = "${local.system_name}-mysql-rg"
  location = var.region
  tags     = local.all_tags
}
...
resource "azurerm_mysql_flexible_server" "this" {
  ...
  geo_redundant_backup_enabled = try(var.settings.geo_redundant_backup, "Disabled") == "Enabled"
  ...
  storage {
    size_gb           = try(var.settings.storage_mb, 20480) / 1024
    iops              = try(var.settings.storage_iops, 396)
    auto_grow_enabled = try(var.settings.storage_auto_grow, "Enabled") == "Enabled"
  }
...
resource "azurerm_mysql_flexible_database" "initial" {
...
resource "azurerm_mysql_flexible_server_configuration" "params" {
